...
class YieldSimulator():
    ...
    def set_random_variables(self):
        ...
        # Determine real-world parameters for estimating initial (u) and current (c) price-per-share (vault and pool details)
    ...
    def run_simulation(self, override_dict=None):
        ...
        if override_dict is not None and 'init_share_price' in override_dict.keys(): # \mu variable
            ...
            self.init_share_price = (1 + self.vault_apy[0]/100)**self.init_vault_age
        ...
        self.market = Market(
            base_asset=init_base_asset_reserves,
            token_asset=init_token_asset_reserves,
            fee_percent=self.fee_percent,
            time_remaining=self.days_until_maturity / (365 * self.t_stretch), # stored as time_remaining inside Market class # TODO: functionalize this calculation
        ...
        self.step_size = self.market.time_remaining / self.days_until_maturity
        ...
        for day in range(0, self.num_trading_days):
            ...
            while day_trading_volume < self.target_daily_volume:
                ...
                (base_asset_reserves, token_asset_reserves) = (self.market.base_asset, self.market.token_asset) # in token units
                if self.trade_direction == 'in':
                    target_reserves = token_asset_reserves if self.token_in == 'fyt' else base_asset_reserves # Assumes 'in' direction
                elif self.trade_direction == 'out':
                    target_reserves = base_asset_reserves if self.token_in == 'fyt' else token_asset_reserves # Assumes 'out' direction
                ...
                if self.verbose:
                    print(f'trades={self.market.base_asset_orders+self.market.token_asset_orders} (price_pe_share,init_share_price)=({self.market.share_price}, '
                        +f'{self.market.init_share_price})amount={self.trade_amount} reserves={(base_asset_reserves,token_asset_reserves)}')
    ...
    def update_analysis_dict(self):
        ...
        days_remaining = self.market.calc_days_remaining(self.days_until_maturity, self.day)
...
class Market():
    ...
    def __init__(self, base_asset, token_asset, fee_percent, time_remaining, pricing_model, init_share_price=1, share_price=1, verbose=False):
    ...
    def calc_days_remaining(self, days_until_maturity, current_day):
        """Calculate the normalized position within the current pool duration"""
        return days_until_maturity - current_day + 1

    def apy(self, days_until_maturity):
        """Returns current APY given the market conditions and pricing model"""
        price = self.pricing_model.calc_spot_price(self.base_asset, self.token_asset, self.total_supply, self.time_remaining, self.init_share_price, self.share_price)
    ...
    def spot_price(self):
        ...
        return self.pricing_model.calc_spot_price(self.base_asset, self.token_asset, self.total_supply, self.time_remaining, self.init_share_price, self.share_price)
    ...
    def check_fees(self, amount, direction, token_in, token_out, in_reserves, out_reserves, trade_results):
        ...
        if self.verbose and self.base_asset_orders + self.token_asset_orders < 10:
            ...
            print(f'amount={amount}, token_asset+total_supply={self.token_asset + self.total_supply}, '
                +f'base_asset/share_price={self.base_asset / self.share_price}, token_in={token_in}, fee_percent={self.fee_percent}, time_remaining={self.time_remaining}, init_share_price={self.init_share_price}, share_price={self.share_price}')
        ...
        if self.verbose and any([
            ...
            print(f'token_asset+total_supply={self.token_asset + self.total_supply}, base_asset/share_price={self.base_asset / self.share_price}, '
                +f'fee_percent={self.fee_percent}, time_remaining={self.time_remaining}, init_share_price={self.init_share_price}, share_price={self.share_price}')
            print(f'without_fee_or_slippage={without_fee_or_slippage}, '
                +f'output_with_fee={output_with_fee}, output_without_fee={output_without_fee}, fee={fee}')
            assert False, (
                ...
                +f'\ninitial_share_price={self.init_share_price}; share_price={self.share_price}; time_remaining={self.time_remaining}'
            )
        if fee < 0:
            max_trade = self.pricing_model.calc_max_trade(in_reserves, out_reserves, self.time_remaining)
            assert False, (
    ...
    def update_market(self, dxdy, slippage, fees, orders, volume):
        """Increments member variables to reflect current market conditions"""
        self.base_asset += dxdy[0]
        self.token_asset += dxdy[1]
        self.cum_base_asset_slippage += slippage[0]
        self.cum_token_asset_slippage += slippage[1]
        self.cum_base_asset_fees += fees[0]
        self.cum_token_asset_fees += fees[1]
        self.base_asset_orders += orders[0]
        self.token_asset_orders += orders[1]
        self.base_asset_volume += volume[0]
        self.token_asset_volume += volume[1]
    ...
    def swap(self, amount, direction, token_in, token_out):
        ...
        if direction == "in":
            if token_in == "fyt" and token_out == "base":
                ...
                trade_results = self.pricing_model.calc_in_given_out(
                    amount, in_reserves, out_reserves, token_in, self.fee_percent, self.time_remaining, self.init_share_price, self.share_price)
                (without_fee_or_slippage, output_with_fee, output_without_fee, fee) = trade_results
                dx = -output_with_fee
                dy = amount
            ...
            if token_in == "fyt" and token_out == "base":
                ...
                trade_results = self.pricing_model.calc_out_given_in(
                    amount, in_reserves, out_reserves, token_out, self.fee_percent, self.time_remaining, self.init_share_price, self.share_price)
                (without_fee_or_slippage, output_with_fee, output_without_fee, fee) = trade_results
                dx = -output_with_fee
                dy = amount
                d_base_asset_slippage = abs(without_fee_or_slippage - output_without_fee)
        ...
        self.update_market((dx, dy), (d_base_asset_slippage, d_token_asset_slippage), (d_base_asset_fee, d_token_asset_fee), (d_base_asset_orders, d_token_asset_orders), (d_base_asset_volume, d_token_asset_volume))
...
class PricingModel(object):
    ...
    def calc_in_given_out(self, out, in_reserves, out_reserves, token_in, fee_percent, time_remaining, init_share_price, share_price):
        """Calculate fees and asset quantity adjustments"""
        raise NotImplementedError

    def calc_out_given_in(self, in_, in_reserves, out_reserves, token_out, fee_percent, time_remaining, init_share_price, share_price):
        """Calculate fees and asset quantity adjustments"""
        raise NotImplementedError
    ...
    def calc_tokens_in_given_lp_out(lp_out, base_asset_reserves, token_asset_reserves, total_supply):
        ...
        if total_supply == 0:
            ...
            token_asset_needed = (lp_out * base_asset_reserves) / ((base_asset_reserves / token_asset_reserves) * total_supply)
    ...
    @staticmethod
    def calc_lp_out_given_tokens_in(base_asset_in, token_asset_in, base_asset_reserves, token_asset_reserves, total_supply):
    ...
    @staticmethod
    def calc_lp_in_given_tokens_out(min_base_asset_out, min_token_asset_out, base_asset_reserves, token_asset_reserves, total_supply):
    ...
    def calc_tokens_out_for_lp_in(lp_in, base_asset_reserves, token_asset_reserves, total_supply):
        ...
        # Solve for y_needed: lp_out = ((x_reserves / y_reserves) * y_needed * total_supply)/x_reserves
        token_asset_needed = (lp_in * base_asset_reserves) / ((base_asset_reserves / token_asset_reserves) * total_supply)
        # Solve for x_needed: x_reserves/y_reserves = x_needed/y_needed
        base_asset_needed = (base_asset_reserves / token_asset_reserves) * token_asset_needed
        return (base_asset_needed, token_asset_needed)

    @staticmethod
    def calc_k_const(in_reserves, out_reserves, time_remaining, scale=1):
        """Returns the 'k' constant variable for trade mathematics"""
        return scale * in_reserves**(1 - time_remaining) + out_reserves**(1 - time_remaining)
    ...
    def calc_max_trade(self, in_reserves, out_reserves, time_remaining):
        ...
        k = self.calc_k_const(in_reserves, out_reserves, time_remaining)#in_reserves**(1 - t) + out_reserves**(1 - t)
        return k**(1 / (1 - time_remaining)) - in_reserves
    ...
    def apy(self, price, days_until_maturity):
        ...
        normalized_pool_duration = days_until_maturity / 365 # TODO: Should this be stretched? if not, rename to make that clear?
        return (1 - price) / price / normalized_pool_duration * 100 # APYW

    def calc_spot_price(self, base_asset_reserves, token_asset_reserves, total_supply, time_remaining, init_share_price=1, share_price=1):
        """Returns the spot price given the current supply and temporal position along the yield curve"""
        return 1 / pow(share_price * (token_asset_reserves + total_supply) / (init_share_price * base_asset_reserves), time_remaining)
    ...
    def calc_apy_from_reserves(self, base_asset_reserves, token_asset_reserves, total_supply, time_remaining, t_stretch, init_share_price=1, share_price=1):
        """Returns the apy given reserve amounts"""
        spot_price = self.calc_spot_price(base_asset_reserves, token_asset_reserves, total_supply, time_remaining, init_share_price, share_price)
        days_until_maturity = time_remaining * 365 * t_stretch # TODO: functionalize this calculation
        return self.apy(spot_price, days_until_maturity)

    def calc_liquidity(self, target_liquidity, market_price, apy, days_until_maturity, time_stretch, init_share_price=1, share_price=1):
        """Returns the reserve volumes and liquidity amounts"""
        spot_price = self.calc_spot_price_from_apy(apy, days_until_maturity)
        time_remaining = days_until_maturity / (365 * time_stretch) # TODO: functionalize this calculation
        ...
        base_asset_reserves = self.calc_base_asset_reserves(
                apy, token_asset_reserves, days_until_maturity, time_stretch, init_share_price, share_price)
        scale_up_factor = target_liquidity / (base_asset_reserves * market_price + token_asset_reserves * market_price * spot_price)
        ...
        if self.verbose:
            ...
            actual_apy = self.calc_apy_from_reserves(
                base_asset_reserves, token_asset_reserves, total_supply, time_remaining, time_stretch, init_share_price, share_price)
            print(f'base_asset_reserves={base_asset_reserves} token_asset={token_asset_reserves} total={liquidity} apy={actual_apy}')
        return (base_asset_reserves, token_asset_reserves, liquidity)

    def calc_base_asset_reserves(self, apy, token_asset_reserves, days_until_maturity, time_stretch, init_share_price, share_price):
        ...
        apy_decimal = apy / 100
        result = 2 * share_price * token_asset_reserves / (init_share_price * (-1 / (apy_decimal * normalized_pool_duration - 1))**(time_stretch_exp) - share_price)
        if self.verbose:
...
class ElementPricingModel(PricingModel):
    ...
    def calc_in_given_out(self, out, in_reserves, out_reserves, token_in, fee_percent, time_remaining, init_share_price=1, share_price=1):
        k = self.calc_k_const(in_reserves, out_reserves, time_remaining) # in_reserves**(1 - t) + out_reserves**(1 - t)
        without_fee = pow(k - pow(out_reserves - out, 1 - time_remaining), 1 / (1 - time_remaining)) - in_reserves
        if token_in == "base":
    ...
    def calc_out_given_in(self, in_, in_reserves, out_reserves, token_out, fee_percent, time_remaining, init_share_price=1, share_price=1):
        k = self.calc_k_const(in_reserves, out_reserves, time_remaining) # in_reserves**(1 - t) + out_reserves**(1 - t)
        without_fee = out_reserves - pow(k - pow(in_reserves + in_, 1 - time_remaining), 1 / (1 - time_remaining))
        if token_out == "base":
    ...
    def calc_base_asset_reserves(self, apy, token_asset_reserves, days_until_maturity, time_stretch, init_share_price=1, share_price=1):
...
class YieldSpacev2PricingModel(PricingModel):
    ...
    def calc_in_given_out(self, out, in_reserves, out_reserves, token_in, fee_percent, time_remaining, init_share_price, share_price):
        ...
        if token_in == "base": # calc shares in for fyt out
            dy = out
            z = in_reserves / share_price # convert from base_asset to z (x=cz)
            token_asset = out_reserves
            # AMM math
            k = self.calc_k_const(init_share_price * z, token_asset, time_remaining, scale)#scale * (u * z)**(1 - t) + y**(1 - t)
            without_fee = (1 / init_share_price * ((k - (token_asset - dy)**(1 - time_remaining)) / scale)**(1 / (1 - time_remaining)) - z) * share_price
            # Fee math
            fee = (out - without_fee) * fee_percent
            with_fee = without_fee + fee
            without_fee_or_slippage = (in_reserves / (share_price / init_share_price * out_reserves))**time_remaining * out

        elif token_in == "fyt": # calc fyt in for shares out
            dz = out / share_price
            z = out_reserves / share_price # convert from base_asset to z (x=cz)
            token_asset = in_reserves
            # AMM math
            k = self.calc_k_const(init_share_price*z, token_asset, time_remaining, scale) # scale * (u * z)**(1 - t) + y**(1 - t)
            without_fee = (k - scale * (init_share_price * z - init_share_price * dz)**(1 - time_remaining))**(1 / (1 - time_remaining)) - token_asset
            # Fee math
            fee = (without_fee - out) * fee_percent
            with_fee = without_fee + fee
            without_fee_or_slippage = ((share_price / init_share_price * in_reserves) / out_reserves)**time_remaining * out
        return (without_fee_or_slippage, with_fee, without_fee, fee)
    ...
    def calc_out_given_in(self, in_, in_reserves, out_reserves, token_out, fee_percent, time_remaining, init_share_price, share_price):
        ...
        if token_out == "base": # calc shares out for fyt in
            ...
            # AMM math
            k = self.calc_k_const(init_share_price * z, token_asset, time_remaining, scale) # scale * (u * z)**(1 - t) + y**(1 - t)
            without_fee = (z - 1 / init_share_price * ((k - (token_asset + dy)**(1 - time_remaining)) / scale)**(1 / (1 - time_remaining))) * share_price
            # Fee math
            ...
            with_fee = without_fee - fee
            without_fee_or_slippage = 1 / ((share_price / init_share_price * in_reserves) / out_reserves)**time_remaining * in_

        elif token_out == "fyt": # calc fyt out for shares in
            dz = in_ / share_price # convert from base_asset to z (x=cz)
            z = in_reserves / share_price # convert from base_asset to z (x=cz)
            token_asset = out_reserves
            # AMM math
            k = self.calc_k_const(init_share_price * z, token_asset, time_remaining, scale) # scale * (u * z)**(1 - t) + y**(1 - t)
            without_fee = token_asset - (k - scale * (init_share_price * z + init_share_price * dz)**(1 - time_remaining))**(1 / (1 - time_remaining))
            # Fee math
            ...
            if fee / in_ < self.floor_fee / 100 / 100:
                fee = in_ * self.floor_fee / 100 / 100
            with_fee = without_fee - fee
            without_fee_or_slippage = 1 / (in_reserves / (share_price / init_share_price * out_reserves))**time_remaining * in_
        return (without_fee_or_slippage, with_fee, without_fee, fee)
