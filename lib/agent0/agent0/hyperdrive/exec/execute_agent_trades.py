...
async def async_execute_single_agent_trade(
    ...
    if liquidate:
        trades: list[
            types.Trade[HyperdriveMarketAction]
        ] = agent.get_liquidation_trades()
    else:
        trades: list[types.Trade[HyperdriveMarketAction]] = agent.get_trades(
    ...
    base_nonce = retry_call(
        5, None, hyperdrive.web3.eth.get_transaction_count, agent.checksum_address
    )
    ...
    wallet_deltas_or_exception: list[
        HyperdriveWalletDeltas | BaseException
    ] = await asyncio.gather(
        *[
            async_match_contract_call_to_trade(
                agent, hyperdrive, trade_object, nonce=Nonce(base_nonce + i)
            )
    ...
    for result, trade_object in zip(wallet_deltas_or_exception, trades):
        if isinstance(result, HyperdriveWalletDeltas):
            ...
            trade_result = TradeResult(
                status=TradeStatus.SUCCESS, agent=agent, trade_object=trade_object
            )
        elif isinstance(result, BaseException):
            trade_result = build_crash_trade_result(
                result, agent, trade_object, hyperdrive
            )
...
async def async_execute_agent_trades(
    ...
    gathered_trade_results: list[list[TradeResult]] = await asyncio.gather(
        *[
            async_execute_single_agent_trade(agent, hyperdrive, liquidate)
            for agent in agents
    ...
    match trade.action_type:
        ...
        case HyperdriveActionType.OPEN_LONG:
            ...
            wallet_deltas = HyperdriveWalletDeltas(
                ...
                longs={
                    trade_result.maturity_time_seconds: Long(trade_result.bond_amount)
                },
            )
        ...
        case HyperdriveActionType.CLOSE_LONG:
            if not trade.maturity_time:
                raise ValueError(
                    "Maturity time was not provided, can't close long position."
                )
        ...
        case HyperdriveActionType.REMOVE_LIQUIDITY:
            trade_result = await hyperdrive.async_remove_liquidity(
                agent, trade.trade_amount, nonce=nonce
            )
            wallet_deltas = HyperdriveWalletDeltas(
                balance=Quantity(
                    amount=trade_result.base_amount,
        ...
        case HyperdriveActionType.REDEEM_WITHDRAW_SHARE:
            trade_result = await hyperdrive.async_redeem_withdraw_shares(
                agent, trade.trade_amount, nonce=nonce
            )
            wallet_deltas = HyperdriveWalletDeltas(
                balance=Quantity(
