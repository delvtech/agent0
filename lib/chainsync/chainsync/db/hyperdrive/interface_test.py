...
class TestTransactionInterface:
    ...
    def test_latest_block_number(self, session):
        ...
        add_transactions([transaction_1], session)

        latest_block_number = get_latest_block_number_from_table(HyperdriveTransaction, session)
        ...
        add_transactions([transaction_2, transaction_3], session)
    ...
    def test_get_transactions(self, session):
        ...
        add_transactions([transaction_1, transaction_2, transaction_3], session)

        transactions_df = get_transactions(session)
    ...
    def test_block_query_transactions(self, session):
        ...
        transactions_df = get_transactions(session, start_block=1)
        ...
        transactions_df = get_transactions(session, end_block=1)
        ...
        transactions_df = get_transactions(session, start_block=1, end_block=-1)
...
class TestPoolConfigInterface:
    ...
    def test_get_pool_config(self, session):
        ...
        add_pool_config(pool_config_1, session)

        pool_config_df_1 = get_pool_config(session, coerce_float=False)
        ...
        add_pool_config(pool_config_2, session)

        pool_config_df_2 = get_pool_config(session, coerce_float=False)
    ...
    def test_primary_id_query_pool_config(self, session):
        ...
        add_pool_config(pool_config, session)

        pool_config_df_1 = get_pool_config(session, contract_address="0", coerce_float=False)
        assert len(pool_config_df_1) == 1
        assert pool_config_df_1.loc[0, "initialSharePrice"] == Decimal("3.2")

        pool_config_df_2 = get_pool_config(session, contract_address="1", coerce_float=False)
    ...
    def test_pool_config_verify(self, session):
        ...
        with pytest.raises(ValueError):
            add_pool_config(pool_config_3, session)
...
class TestPoolInfoInterface:
    ...
    def test_latest_block_number(self, session):
        ...
        add_pool_infos([pool_info_1], session)

        latest_block_number = get_latest_block_number_from_pool_info_table(session)
        assert latest_block_number == 1
        ...
        add_pool_infos([pool_info_1, pool_info_2], session)
    ...
    def test_get_pool_info(self, session):
        ...
        add_pool_infos([pool_info_1, pool_info_2, pool_info_3], session)

        pool_info_df = get_pool_info(session)
        np.testing.assert_array_equal(
    ...
    def test_block_query_pool_info(self, session):
        ...
        pool_info_3 = PoolInfo(blockNumber=2, timestamp=timestamp_3)
        add_pool_infos([pool_info_1, pool_info_2, pool_info_3], session)
        pool_info_df = get_pool_info(session, start_block=1)
        np.testing.assert_array_equal(
        ...
        pool_info_df = get_pool_info(session, end_block=1)
        ...
        pool_info_df = get_pool_info(session, start_block=1, end_block=-1)
...
class TestWalletDeltaInterface:
    ...
    def test_latest_block_number(self, session):
        ...
        add_wallet_deltas([wallet_delta_1], session)
        latest_block_number = get_latest_block_number_from_table(WalletDelta, session)
        assert latest_block_number == 1
        ...
        add_wallet_deltas([wallet_delta_2, wallet_delta_3], session)
        latest_block_number = get_latest_block_number_from_table(WalletDelta, session)
        assert latest_block_number == 3
    ...
    def test_get_wallet_delta(self, session):
        ...
        add_wallet_deltas([wallet_delta_1, wallet_delta_2, wallet_delta_3], session)
        wallet_delta_df = get_wallet_deltas(session)
    ...
    def test_block_query_wallet_delta(self, session):
        ...
        wallet_delta_3 = WalletDelta(blockNumber=2, transactionHash="a", delta=Decimal("3.3"))
        add_wallet_deltas([wallet_delta_1, wallet_delta_2, wallet_delta_3], session)
        wallet_delta_df = get_wallet_deltas(session, start_block=1)
        ...
        wallet_delta_df = get_wallet_deltas(session, end_block=1)
        ...
        wallet_delta_df = get_wallet_deltas(session, start_block=1, end_block=-1)
