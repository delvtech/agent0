...
from elf_simulations.data.hyperdrive.db_schema import WalletInfo
...
class TestWalletInfoInterface:
    ...
    def test_latest_block_number(self, session):
        ...
        elf_simulations.data.hyperdrive.postgres.add_wallet_infos([wallet_info_1], session)
        ...
        elf_simulations.data.hyperdrive.postgres.add_wallet_infos([wallet_info_2, wallet_info_3], session)
    ...
    def test_get_wallet_info(self, session):
        ...
        elf_simulations.data.hyperdrive.postgres.add_wallet_infos(
            [wallet_info_1, wallet_info_2, wallet_info_3], session
        )

        wallet_info_df = elf_simulations.data.hyperdrive.postgres.get_all_wallet_info(session)
    ...
    def test_block_query_wallet_info(self, session):
        ...
        wallet_info_df = elf_simulations.data.hyperdrive.postgres.get_all_wallet_info(session, start_block=1)
        ...
        wallet_info_df = elf_simulations.data.hyperdrive.postgres.get_all_wallet_info(session, end_block=1)
        ...
        wallet_info_df = elf_simulations.data.hyperdrive.postgres.get_all_wallet_info(
    ...
    def test_current_wallet_info(self, session):
        ...
        elf_simulations.data.hyperdrive.postgres.add_wallet_infos([wallet_info_1, wallet_info_2], session)

        wallet_info_df = elf_simulations.data.hyperdrive.postgres.get_current_wallet_info(session).reset_index()
        ...
        elf_simulations.data.hyperdrive.postgres.add_wallet_infos([wallet_info_3], session)
        wallet_info_df = elf_simulations.data.hyperdrive.postgres.get_current_wallet_info(session).reset_index()
        np.testing.assert_array_equal(wallet_info_df["tokenType"], ["BASE", "LP"])
    ...
    def test_get_agents(self, session):
        ...
        agents = elf_simulations.data.hyperdrive.postgres.get_agents(session)
        assert len(agents) == 2
