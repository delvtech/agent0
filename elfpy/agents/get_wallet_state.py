...
def get_wallet_state(agent_wallet: wallet.Wallet, market: hyperdrive_market.Market) -> dict[str, float]:
    ...
    if agent_wallet.lp_tokens > 0 and market.market_state.lp_total_supply > 0:
    ...
    return {
        ...
        f"agent_{agent_wallet.address}_num_longs": len(agent_wallet.longs),
        f"agent_{agent_wallet.address}_num_shorts": len(agent_wallet.shorts),
