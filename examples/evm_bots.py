...
def set_up_ape(
    ...
    return provider, automine, base_instance, hyperdrive_instance, hyperdrive_config, agent_addresses, trade_history
    ...
    for trade_object in trades:
        try:
            ...
            if bot_config.halt_on_errors:
                ...
                # rename trades_regular.parquet to trades_crash.parquet
                trades_regular = bot_config.scratch["project_dir"] / "trades_regular.parquet"
                trades_crash = bot_config.scratch["project_dir"] / "trades_crash.parquet"
                Path.rename(trades_regular, trades_crash)
                logging.info(
...
def dump_state(bot_config, block_number, rng, addresses, sim_agents, hyperdrive_instance, trade_history=None):
    ...
    trade_history_to_be_dumped = trade_history.copy()
    convert_cols = [
        "block_number",
        "log_index",
        "transaction_index",
    ...
    for col in convert_cols:
        trade_history_to_be_dumped.loc[:, col] = trade_history.loc[:, col].astype(str)
    trade_history_to_be_dumped.to_parquet(
        bot_config.scratch["project_dir"] / "trades_regular.parquet", engine="pyarrow"
    )
