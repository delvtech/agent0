...
class User:
    ...
    def stochastic_direction(pool_apy,pool_apy_target_range,days_trades,pool_apy_target_range_convergence_speed,rng,
        ...
        apy_distance_in_target_range = np.clip(
            (pool_apy - pool_apy_target_range[0])
            / (pool_apy_target_range[1] - pool_apy_target_range[0]),
            0,
            1,
        )
        ...
        actual_convergence_strength = (
            0.5
            + (pool_apy_target_range_convergence_speed - 0.5)
        ...
        expected_proportion = (
            ...
            else 1 - actual_convergence_strength
        )
        ...
        if streak_luck > 0.98 and verbose:
            ...
            if 0 < apy_distance_from_mid_when_in_range < 1:
                actual_convergence_strength = (
                    actual_convergence_strength
                    + (1 - actual_convergence_strength) * streak_luck**1.5
                )  # force convergence when on bad streaks
            token_index = (
                convergence_direction
                if rng.random() < actual_convergence_strength
