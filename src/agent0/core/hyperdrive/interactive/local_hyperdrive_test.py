...
def _ensure_db_wallet_matches_agent_wallet(in_hyperdrive: LocalHyperdrive, agent_wallet: HyperdriveWallet):
    ...
    # Test against db
    current_positions_df = in_hyperdrive.get_current_positions(coerce_float=False)

    base_wallet_df = current_positions_df[current_positions_df["base_token_type"] == BASE_TOKEN_SYMBOL]
    assert len(base_wallet_df) == 1
    assert agent_wallet.balance.amount == FixedPoint(base_wallet_df.iloc[0]["position"])

    # Check lp
    lp_wallet_df = current_positions_df[current_positions_df["base_token_type"] == "LP"]
    ...
    # Check longs
    long_wallet_df = current_positions_df[current_positions_df["base_token_type"] == "LONG"]
    ...
    # Check shorts
    short_wallet_df = current_positions_df[current_positions_df["base_token_type"] == "SHORT"]
    ...
    withdrawal_wallet_df = current_positions_df[current_positions_df["base_token_type"] == "WITHDRAWAL_SHARE"]
...
def test_save_load_snapshot(chain_fixture: LocalChain):
    ...
    init_db_positions = interactive_hyperdrive.get_current_positions(coerce_float=False).copy()
    ...
    check_db_positions = interactive_hyperdrive.get_current_positions(coerce_float=False)
    ...
    assert not check_db_positions.equals(init_db_positions)
    ...
    assert check_base_on_chain == init_base_on_chain
    assert check_agent_wallet == init_agent_wallet
    assert check_db_positions.equals(init_db_positions)
    assert check_pool_info_on_chain == init_pool_info_on_chain
...
def test_liquidate(fast_chain_fixture: LocalChain):
    ...
    current_positions = interactive_hyperdrive.get_current_positions()
    assert current_positions.shape[0] == 4  # we have 4 open positions, including base
    alice.liquidate()
    current_positions = interactive_hyperdrive.get_current_positions()
    assert current_positions.shape[0] == 1  # we have 1 open position, including base
