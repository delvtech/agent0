...
from agent0.ethpy.hyperdrive import BASE_TOKEN_SYMBOL, AssetIdPrefix, HyperdriveReadInterface, encode_asset_id
...
def _ensure_db_wallet_matches_agent_wallet(in_hyperdrive: LocalHyperdrive, agent_wallet: HyperdriveWallet):
    # NOTE this function is assuming only one agent is making trades

    # Test against db
    current_wallet_df = in_hyperdrive.get_current_wallet(coerce_float=False)
...
def test_funding_and_trades(fast_chain_fixture: LocalChain):
    ...
    interactive_hyperdrive_2 = LocalHyperdrive(fast_chain_fixture, initial_pool_config)
    ...
    assert (hyperdrive_agent0.get_pool_positions().balance.amount) == FixedPoint(1_111_111)
    assert (hyperdrive_agent1.get_pool_positions().balance.amount) == FixedPoint(222_222)
    assert (hyperdrive_agent2.get_pool_positions().balance.amount) == FixedPoint(333_333)
    ...
    assert hyperdrive_agent0.get_pool_positions().lp_tokens == add_liquidity_event.lp_amount
    _ensure_db_wallet_matches_agent_wallet(interactive_hyperdrive, hyperdrive_agent0.get_pool_positions())
    ...
    assert hyperdrive_agent0.get_pool_positions().lp_tokens == FixedPoint(0)
    assert hyperdrive_agent0.get_pool_positions().withdraw_shares == remove_liquidity_event.withdrawal_share_amount
    _ensure_db_wallet_matches_agent_wallet(interactive_hyperdrive, hyperdrive_agent0.get_pool_positions())
    ...
    agent0_shorts = list(hyperdrive_agent0.get_pool_positions().shorts.values())
    ...
    assert len(hyperdrive_agent0.get_pool_positions().longs) == 0
    _ensure_db_wallet_matches_agent_wallet(interactive_hyperdrive, hyperdrive_agent0.get_pool_positions())
    ...
    assert len(hyperdrive_agent0.get_pool_positions().shorts) == 0
    _ensure_db_wallet_matches_agent_wallet(interactive_hyperdrive, hyperdrive_agent0.get_pool_positions())
    ...
    assert hyperdrive_agent0.get_pool_positions().withdraw_shares == FixedPoint(0)
    _ensure_db_wallet_matches_agent_wallet(interactive_hyperdrive, hyperdrive_agent0.get_pool_positions())
...
def test_save_load_snapshot(chain_fixture: LocalChain):
    ...
    init_agent_wallet = hyperdrive_agent.get_pool_positions().copy()
    ...
    check_agent_wallet = hyperdrive_agent.get_pool_positions()
...
def test_access_deployer_liquidity(fast_chain_fixture: LocalChain):
    ...
    assert (
        ...
        == larry.get_pool_positions().lp_tokens
    )
    # Hyperdrive pool steals 2 * minimumShareReserves from the initial deployer's liquidity
    assert larry.get_pool_positions().lp_tokens == config.initial_liquidity - 2 * config.minimum_share_reserves
...
def test_remove_deployer_liquidity(fast_chain_fixture: LocalChain):
    ...
    larry.remove_liquidity(shares=larry.get_pool_positions().lp_tokens)
    assert larry.get_pool_positions().lp_tokens == 0
    assert (
        FixedPoint(
            scaled_value=interactive_hyperdrive.interface.hyperdrive_contract.functions.balanceOf(
                encode_asset_id(AssetIdPrefix.LP, 0),
