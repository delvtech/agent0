...
from agent0.ethpy.hyperdrive import HyperdriveReadWriteInterface, get_hyperdrive_addresses_from_uri
...
def _setup_agents(
    ...
    if load_wallet_state:
        if eth_config.database_api_uri is not None:
            ...
            for agent in agent_accounts:
                agent.wallet = build_wallet_positions_from_chain(agent, interface)
