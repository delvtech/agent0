"""Helper function to get agent accounts from a config."""
...
def get_agent_accounts(
    ...
    for agent_info in agent_config:
        for policy_instance_index in range(agent_info.number_of_agents):  # instantiate one agent per policy
            ...
            if get_account_balance(interface.web3, eth_agent.checksum_address) == 0:
    ...
    asyncio.run(_set_max_approval(agents, interface))
