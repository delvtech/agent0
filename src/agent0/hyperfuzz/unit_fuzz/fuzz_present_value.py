...
def fuzz_present_value(
    ...
    for trade_type in [
        ...
        if agent.get_pool_positions().balance.amount < FixedPoint("1e10"):
            agent.add_funds(base=FixedPoint("1e10") - agent.get_pool_positions().balance.amount)

        # Set up trade amount bounds
        min_trade = interactive_hyperdrive.interface.pool_config.minimum_transaction_amount
        max_budget = agent.get_pool_positions().balance.amount
        trade_amount = None
        ...
        match trade_type:
            ...
            case HyperdriveActionType.CLOSE_LONG:
                maturity_time, open_trade = next(iter(agent.get_pool_positions().longs.items()))
            ...
            case HyperdriveActionType.CLOSE_SHORT:
                maturity_time, open_trade = next(iter(agent.get_pool_positions().shorts.items()))
            ...
            case HyperdriveActionType.ADD_LIQUIDITY:
                ...
                trade_amount = FixedPoint(
                    scaled_value=int(
                        np.floor(
                            rng.uniform(
                                low=min_trade.scaled_value, high=agent.get_pool_positions().balance.amount.scaled_value
                            )
                        )
                    )
                )
            ...
            case HyperdriveActionType.REMOVE_LIQUIDITY:
                ...
                trade_amount = agent.get_pool_positions().lp_tokens
                trade_event = agent.remove_liquidity(agent.get_pool_positions().lp_tokens)
            case _:
