                ...
                for yba in ybas:
                    ...
                    for PricingModel in PricingModelList:
                        ...
                        while m.t > -epsilon:
                            ...
                            todays_target_volume = np.random.normal(ub/2,ub/10) if day<days_until_maturity else 
                            while todays_target_volume > todays_volume:
                                ...
                                while fee < 0:
                                    ...
                                    cols = ['Run_ID',"model_name","init.apr","init.percent_fee","init.days_until_maturity","init.max_order_size","init.time_stretch"\
                                        ...
                                        ,"input.base_market_price","input.unit_fyt_price","input.apr","input.base_reserves","input.fyt_reserves","input.trade_number"\
                                    ...
                                    trade = [run_id,model_name,APR,g,days_until_maturity,max_order_size,time_stretch\
                                ...
                                todays_num_trades += 1
                            # print("\tDay: " + str(day) + " PT Price: " + str(m.spot_price()) + " Implied apr: " + str(m.apr(days_until_maturity-day+1)) + " Target Volume Factor: {:,.4f}".format(math.log10(1/maturity_ratio)) \
...
df['pr']=((df.loc[:,'input.unit_fyt_price']-df.loc[0,'input.unit_fyt_price'])/1) # this is APR (does not include compounding)
df['pu']=(df.loc[:,'pr']+1)*df.loc[:,'input.u'] # this is APR (does not include compounding)
display(df.loc[:,['input.day','input.unit_fyt_price','pr','pu']])
...
for idx,_df in enumerate(dfs):
  ...
  for model in df_fees_volume.model_name.unique():
    ax[currentPlot] = _df.loc[_df.model_name==model,:]\
      .plot(x="input.trade_number",y="input.apr",figsize=(24,18),ax=ax[currentPlot],label=model)
  ...
  ax[currentPlot] = _df.loc[_df.model_name==model,:].plot(x="input.trade_number",y="input.vault_apr",figsize=(24,18),ax=ax[currentPlot],label='vault_apr {}→{}'.format(_df.loc[_df.model_name==model,'input.vault_apr'].values[0],_df.loc[_df.model_name==model,'input.vault_apr'].values[-1]))
  ...
  ax[currentPlot] = _df.loc[_df.model_name==model,:].plot(x="input.trade_number",y="input.c",figsize=(24,18),ax=ax[currentPlot],label='c {:,.3f}→{:,.3f} r={:.3%}'.format(_df.loc[_df.model_name==model,'input.c'].values[0],_df.loc[_df.model_name==model,'input.c'].values[-1],_df.loc[_df.model_name==model,'input.c'].values[-1]/_df.loc[_df.model_name==model,'input.c'].values[0]-1))
  ax[currentPlot] = _df.loc[_df.model_name==model,:].plot(x="input.trade_number",y="input.u",figsize=(24,18),ax=ax[currentPlot],label='u')
  ax[currentPlot] = _df.loc[_df.model_name==model,:].plot(x="input.trade_number",y="pu",figsize=(24,18),ax=ax[currentPlot],label='p {:,.3f}→{:,.3f} r={:.3%}'.format(_df.loc[_df.model_name==model,'pu'].values[0],_df.loc[_df.model_name==model,'pu'].values[-1],_df.loc[_df.model_name==model,'pu'].values[-1]/_df.loc[_df.model_name==model,'pu'].values[0]-1))
  # ax2 = _df.loc[_df.model_name==model,:].plot(secondary_y=True,x="input.trade_number",y='input.unit_fyt_price',figsize=(24,18),ax=ax[currentPlot],label='p')
